...
**Hint**: To run IGV, you can open a terminal, and type <code style="background-color:#eaeaea; padding:2px 3px 3px;white-space:pre-wrap">igv</code>. It will then take a little bit to start.
...
**TASK**: Run qualimap using the command line, using the command: <code style="background-color:#eaeaea; padding:2px 3px 3px;white-space:pre-wrap">qualimap rnaseq -bam mut_lib1_R1.bam -gtf Drosophila_melanogaster.BGP6.85.sample.gtf</code> (you may need to adjust the file names and locations). You should see a new folder created, containing the qualimap report in html format.
<br/>
<br/>
